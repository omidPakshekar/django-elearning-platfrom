...
class ContentApiTestCase(TestCase):
    ...
    def test_content_update(self):
        data = {
            ...
            "item": {
                "id": 1, 
                "title": 'title three meeeeee',
                "content": 'this is text3444', 
            }   
        }
        resp = self.client.put('/api/v1/content/1/', data,  format='json')
        self.assertEqual(resp.status_code, 200)
        resp = resp.json()
        # check data result
        ...
        self.assertEqual(resp["object_id"], content.object_id)
        self.assertEqual(resp["order"], content.order)
        self.assertEqual(resp["item"]['title'], content.item.title)
        self.assertEqual(resp["item"]['content'], content.item.content)
        ...
        self.client.credentials()
        resp = self.client.put('/api/v1/course/1/', data, format='json')
        self.assertEqual(resp.status_code, 403)
    ...
    def test_module_partial_update(self):
        data = {
            "item": {
                "id": 1, 
                "title": 'title 9',
                "content": 'this is text 9', 
            }  
        }
        resp = self.client.patch('/api/v1/content/1/', data, format='json')
        self.assertEqual(resp.status_code, 200)
        resp = resp.json()
        # check result
        content = Content.objects.get(id=1)
